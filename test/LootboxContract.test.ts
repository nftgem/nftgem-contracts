...
describe('Lootbox Contract test suite', () => {
  ...
  describe('Add Loot to Lootbox', () => {
    ...
    it('Should revert if name is not passed', async () => {
      const loot = await getLootData();
      const {LootboxContract} = await createLootbox();
      // set name to ''
      loot[4] = '';
    ...
    it('Should revert if probability is not passed', async () => {
      const loot = await getLootData();
      const {LootboxContract} = await createLootbox();
      // set probability to 0
      loot[5] = 0;
      await expect(LootboxContract.addLoot(loot)).to.be.revertedWith(
        'probability must be set'
      );
    });
  });
  it('Mint Loot', async () => {
  ...
  describe('Open Lootbox', () => {
    it('should open lootbox', async () => {
      const {LootboxContract, NFTGemMultiToken, lootboxHash, owner} =
        await addLoot();
      await LootboxContract.mintLootboxTokens(1000);
      const tx = await LootboxContract.openLootbox();
      expect(tx).to.emit(LootboxContract, 'LootboxOpened');
      // Opening a lootbox should burn one lootbox token
      expect(
        (
          await NFTGemMultiToken.balanceOf(owner.address, lootboxHash)
        ).toNumber()
      ).to.be.equal(999);
      const lootHash = ethers.utils.keccak256(
        ethers.utils.solidityPack(['string', 'string'], ['TEST1', 'SMBL'])
      );
      expect(
        (
          await NFTGemMultiToken.balanceOf(owner.address, lootHash)
        ).toNumber()
      ).to.be.equal(1);
    });
    it('should revert if insufficient lootbox tokens', async () => {
      const {LootboxContract} = await addLoot();
      await expect(LootboxContract.openLootbox()).to.be.revertedWith(
